...
// Hash: cbd4f5e301fc9e7cf60c6f7b66adcf5ccd45a97b733b64cc09e1cf8404e76d37
package eth
...
func (b *BlobsAndCommitments) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (1) 'Commitments'
	if size := len(b.Commitments); size > 16 {
		err = ssz.ErrListTooBigFn("--.Commitments", size, 16)
		return
	}
	for ii := 0; ii < len(b.Commitments); ii++ {
		if size := len(b.Commitments[ii]); size != 48 {
			err = ssz.ErrBytesLengthFn("--.Commitments[ii]", size, 48)
			return
		}
	...
	// Field (1) 'Commitments'
	{
		...
		for ii := 0; ii < num; ii++ {
			if cap(b.Commitments[ii]) == 0 {
				b.Commitments[ii] = make([]byte, 0, len(buf[ii*48:(ii+1)*48]))
			}
			b.Commitments[ii] = append(b.Commitments[ii], buf[ii*48:(ii+1)*48]...)
		}
	}
	return err
}
...
func (p *PolynomialAndCommitment) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'Polynomial'
	if size := len(p.Polynomial); size != 4096 {
		err = ssz.ErrVectorLengthFn("--.Polynomial", size, 4096)
		return
	}
	for ii := 0; ii < 4096; ii++ {
		if size := len(p.Polynomial[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("--.Polynomial[ii]", size, 32)
			return
		}
	...
	// Field (1) 'Commitment'
	if size := len(p.Commitment); size != 48 {
		err = ssz.ErrBytesLengthFn("--.Commitment", size, 48)
		return
	}
...
func (p *PolynomialAndCommitment) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Polynomial'
	p.Polynomial = make([][]byte, 4096)
	for ii := 0; ii < 4096; ii++ {
		if cap(p.Polynomial[ii]) == 0 {
			p.Polynomial[ii] = make([]byte, 0, len(buf[0:131072][ii*32:(ii+1)*32]))
		}
		p.Polynomial[ii] = append(p.Polynomial[ii], buf[0:131072][ii*32:(ii+1)*32]...)
	}

	// Field (1) 'Commitment'
	if cap(p.Commitment) == 0 {
		p.Commitment = make([]byte, 0, len(buf[131072:131120]))
	}
	p.Commitment = append(p.Commitment, buf[131072:131120]...)
